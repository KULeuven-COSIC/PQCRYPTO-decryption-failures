...
int testciphertext(int num_trials, int threshold, ramstake_ciphertext c, ramstake_secret_key sk, int kat)
{
...
int main( int argc, char ** argv )
{
    ...
    if( argc != 3 || strlen(argv[1]) % 2 != 0 )
    {
        printf("usage: ./perform [random seed, eg d13d13deadbeef] [verbose: 1, non verbose: 0]\n");
        return 0;
    }
    ...
    for( ; ; )
    {
        ...
        for( ; ; pos=(pos+distance)%RAMSTAKE_MODULUS_BITSIZE)
        {
            ...
            trial_success = testciphertext(num_trials, threshold, c, sk, 0);
